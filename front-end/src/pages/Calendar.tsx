...
const getEventStyles = (type: CalEvent['type']) => {
  const map = {
    game: { 
      ...
      pill: 'bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-400 border-emerald-500/30',
      icon: Shield,
      label: 'Jogo' 
    },
    training: { 
      bg: 'bg-sky-500/10 text-sky-400 border-sky-500/20',
      pill: 'bg-sky-500/20 hover:bg-sky-500/30 text-sky-300 border-sky-500/30',
      icon: Target,
      label: 'Treino' 
    },
    championship: { 
    ...
    other: { 
      ...
      pill: 'bg-rose-500/20 hover:bg-rose-500/30 text-rose-300 border-rose-500/30',
      icon: Info,
      label: 'Outro' 
    },
  };
...
export function Calendar() {
  ...
  return (
    <div className="max-w-7xl flex flex-col lg:flex-row gap-8 animate-in fade-in duration-500">
      ...
      <div className="flex-1 bg-zinc-900/40 border border-white/5 rounded-2xl p-6">
        
        {/* Controles de Navegação */}
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            <div className="p-2 bg-white/5 rounded-lg border border-white/10 flex items-center justify-center text-zinc-400 hidden sm:flex">
              <CalendarIcon size={24} />
            </div>
            <h2 className="text-2xl font-bold text-zinc-200 capitalize">{monthName}</h2>
          </div>
          ...
          <div className="flex items-center gap-2 bg-zinc-950/50 border border-white/5 rounded-lg p-1">
            ...
            <button 
              ...
              className="px-3 text-xs font-bold text-zinc-500 hover:text-zinc-300 transition-colors uppercase tracking-wider"
            >
              Hoje
            </button>
            <button 
        ...
        <div className="grid grid-cols-7 gap-2 mb-2">
          {['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'].map(d => (
            <div key={d} className="text-center text-xs font-bold text-zinc-500 uppercase tracking-widest py-2">
              {d}
            </div>
          ))}
        </div>
            ...
            return (
              <div 
                ...
                className={`min-h-[100px] p-2 rounded-xl border transition-all
                  ${isTod ? 'bg-emerald-500/10 border-emerald-500/30' : 
                    isCurrentMonth ? 'bg-zinc-950/50 border-white/5 hover:border-white/10' : 
                    'bg-zinc-950/20 border-transparent opacity-50'}
                `}
              >
                <span className={`text-sm font-bold mb-2 flex items-center justify-center w-6 h-6 rounded-full
                  ${isTod ? 'bg-emerald-500 text-zinc-950' : 'text-zinc-400'}
                `}>
                  {cell.day}
                </span>
      ...
      <div className="lg:w-80 flex-shrink-0 flex flex-col gap-6">
        ...
        <button className="flex items-center justify-center gap-2 bg-emerald-500 hover:bg-emerald-600 text-zinc-950 font-bold text-sm py-3 px-4 rounded-xl transition-colors w-full shadow-lg shadow-emerald-500/20">
          <Plus size={18} />
            ...
            {upcomingEvents.map(ev => {
              ...
              return (
                ...
                >
                  ...
                  <div>
                    <p className="text-sm font-bold text-zinc-200 group-hover:text-white transition-colors line-clamp-1">{ev.title}</p>
...
function EventModal({ event, onClose }: { event: CalEvent, onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
      <div 
        className="bg-zinc-900 border border-white/10 rounded-2xl w-full max-w-sm shadow-2xl flex flex-col"
      ...
      >
        ...
        <div className="p-6">
          <h2 className="text-xl font-bold text-zinc-200 mb-4">{event.title}</h2>
          ...
          <div className="mt-6 pt-6 border-t border-white/5">
            <button className="w-full bg-white/5 hover:bg-white/10 text-zinc-200 border border-white/10 font-bold text-sm py-2.5 rounded-xl transition-colors">
              Editar Evento
            </button>
          </div>
        </div>
